...
userSchema.pre("save", async function(next){ //but change password only when password field is modified
...
userSchema.methods.generateRefreshToken=function(){ //refresh token usally has low payload
    return jwt.sign({
        _id: this._id,
    },
