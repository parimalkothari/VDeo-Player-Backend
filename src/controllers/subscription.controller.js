...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    if(!subscribed){
        ...
        const unsub= await Subscription.deleteOne({_id: subscribed._id})
...
const getUserChannelSubscribers = asyncHandler(async (req, res) => {
    const {channelId, sortBy="createdAt",sortType=1,pages=1, limit=3} = req.params
    ...
    res
    .status(200)
    .json(new apiResponse(201,subscribers,"subscribers list fetched successfully"))
})

// controller to return channel list to which user has subscribed
const getSubscribedChannels = asyncHandler(async (req, res) => {
    const { subscriberId, sortBy="createdAt",sortType=1,pages=1, limit=3} = req.params
    const pageInt=parseInt(pages)
    const limitInt=parseInt(limit)
